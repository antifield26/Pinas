...
pub fn spawn_all(pool: &sqlx::SqlitePool, config: &crate::config::Config, cancel: CancellationToken) {
    spawn_temp_chunk_cleanup(pool.clone(), config.temp_cleanup_hours, cancel.child_token());
    spawn_rate_limit_cleanup(cancel.child_token());
    spawn_log_cleanup(cancel.child_token());
    spawn_trash_cleanup(pool.clone(), config.trash_cleanup_days, config.trash_cleanup_interval_hours, cancel.child_token());
...
/// 清理过期回收站条目
fn spawn_trash_cleanup(pool: SqlitePool, days: u32, interval_hours: u64, cancel: CancellationToken) {
    tokio::spawn(async move {
