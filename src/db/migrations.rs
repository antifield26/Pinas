...
pub async fn run(pool: &SqlitePool) {
    ...
    tracing::debug!("[DB] Schema 版本: {} (最新: {})", current.max(CURRENT_VERSION), CURRENT_VERSION);
}
...
async fn apply_v3_migrations(pool: &SqlitePool) {
    let cols: Vec<String> = sqlx::query("PRAGMA table_info(user_settings)")
        ...
        .iter().map(|r| r.get::<String, _>(1)).collect();

    if !cols.iter().any(|c| c == "temperature") {
        sqlx::query("ALTER TABLE user_settings ADD COLUMN temperature REAL NOT NULL DEFAULT 0.7")
            .execute(pool).await.expect("迁移 user_settings.temperature 失败");
    }
    if !cols.iter().any(|c| c == "max_tokens") {
        sqlx::query("ALTER TABLE user_settings ADD COLUMN max_tokens INTEGER NOT NULL DEFAULT 4096")
            .execute(pool).await.expect("迁移 user_settings.max_tokens 失败");
    }
