...
async fn read_cpu_temp() -> f32 {
    tokio::fs::read_to_string("/sys/class/thermal/thermal_zone0/temp").await
        .ok().and_then(|s| s.trim().parse::<f32>().ok()).unwrap_or(0.0) / 1000.0
}
    ...
    if let Ok(mem) = tokio::fs::read_to_string("/proc/meminfo").await {
        for line in mem.lines() {
            if line.starts_with("MemTotal:") {
                ...
                avail = line.split_whitespace().nth(1).and_then(|v| v.parse().ok()).unwrap_or(0) / 1024;
            }
        }
    }
    (total, avail)
...
fn parse_cpu_ticks(stat_content: &str) -> Option<(u64, u64)> {
    let parts: Vec<u64> = stat_content.lines().next()?
        .split_whitespace().skip(1).take(7)
...
async fn read_cpu_ticks() -> Option<(u64, u64)> {
    tokio::fs::read_to_string("/proc/stat").await.ok().as_deref().and_then(parse_cpu_ticks)
}

/// 计算 CPU 使用率百分比（两次采样，间隔 200ms）
async fn calc_cpu_usage() -> f32 {
    if let (Some((a1, t1)), Some((a2, t2))) = (
        read_cpu_ticks().await,
        { tokio::time::sleep(std::time::Duration::from_millis(200)).await; read_cpu_ticks().await }
    ) {
...
pub async fn health_check(
    ...
    let db_ok = sqlx::query_scalar::<_, i64>("SELECT 1").fetch_one(&pool).await.is_ok();
...
pub async fn system_monitor_fragment(
    ...
    if session.role != crate::constants::ROLE_ADMIN {
        return AppTemplate(SystemMonitorLive {
            cpu_bar_pct: 0, mem_used_mb: 0, mem_total_mb: 0,
            mem_bar_pct: 0, temp_display: "N/A".into(), temp_color_class: String::new(),
        }).into_response();
    }
    ...
    let mem_bar_pct = mem_used_mb.checked_mul(100).and_then(|v| v.checked_div(mem_total_mb)).unwrap_or(0) as u32;

    let temp_display = format!("{:.1} C", cpu_temp);
    let temp_color_class = if cpu_temp > 75.0 { "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400" }
        else if cpu_temp > 60.0 { "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400" }
        else { "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" };

    AppTemplate(SystemMonitorLive {
        cpu_bar_pct,
        mem_used_mb, mem_total_mb, mem_bar_pct,
        temp_display, temp_color_class: temp_color_class.to_string(),
    }).into_response()
}