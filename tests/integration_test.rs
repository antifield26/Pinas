...
async fn test_upload_chunk_and_merge() {
    ...
    let check: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert!(!check["exists"].as_bool().unwrap());
    let uploaded: Vec<i32> = check["uploaded_chunks"].as_array().unwrap()
    ...
    let resp = app.clone().oneshot(post_json_with_token(
        "/api/files/merge",
        &format!(r#"{{"identifier":"{}","file_name":"chunked.bin","parent_path":""}}"#, identifier),
        &token,
    )).await.unwrap();
    ...
    // 5. 列表包含该文件
    let resp = app.clone().oneshot(get_with_token("/api/files/list?path=", &token)).await.unwrap();
    let body = axum::body::to_bytes(resp.into_body(), 4096).await.unwrap();
    let list: serde_json::Value = serde_json::from_slice(&body).unwrap();
    let items = list["items"].as_array().unwrap();
    assert!(items.iter().any(|f| f["name"] == "chunked.bin"), "列表应包含合并后的文件");

    // 6. 磁盘文件字节完整性 = 3 × CHUNK_SIZE
    let disk_path = format!("uploads/{}/chunked.bin", username);
    let meta = tokio::fs::metadata(&disk_path).await.expect("磁盘文件应存在");
    assert_eq!(meta.len() as usize, 3 * CHUNK_SIZE, "合并文件字节数应等于分片总和");
    let content = tokio::fs::read(&disk_path).await.unwrap();
    for (i, data) in chunks.iter().enumerate() {
        assert_eq!(&content[i * CHUNK_SIZE..(i + 1) * CHUNK_SIZE], data.as_slice(),
            "分片 {} 内容应原样保留", i);
    }
...
async fn test_upload_exceeds_quota_forbidden() {
    ...
    let resp = app.clone().oneshot(post_json_with_token(
        "/api/files/merge",
        &format!(r#"{{"identifier":"{}","file_name":"quota.bin","parent_path":""}}"#, identifier),
        &token,
    )).await.unwrap();
    assert_eq!(resp.status(), StatusCode::FORBIDDEN, "超配额合并应返回 403");
    let body = axum::body::to_bytes(resp.into_body(), 2048).await.unwrap();
    let text = String::from_utf8_lossy(&body).to_string();
    assert!(text.contains("存储空间不足"), "应返回通用配额提示: {}", text);
}
...
async fn test_share_with_password() {
    ...
    // 1. 无密码访问 → 显示提取码表单
    let resp = app.clone().oneshot(
        Request::builder().method("GET").uri(format!("/s/{}", code)).body(Body::empty()).unwrap(),
    ).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), 4096).await.unwrap();
    ...
    // 2. 错误密码 → 仍回表单
    let resp = app.clone().oneshot(
        Request::builder().method("GET").uri(format!("/s/{}?password=wrongpass", code)).body(Body::empty()).unwrap(),
    ).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), 4096).await.unwrap();
    let html = String::from_utf8_lossy(&body).to_string();
    ...
    // 3. 正确密码 → 显示目录浏览页
    let resp = app.clone().oneshot(
        Request::builder().method("GET").uri(format!("/s/{}?password=secret456", code)).body(Body::empty()).unwrap(),
    ).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
    let body = axum::body::to_bytes(resp.into_body(), 4096).await.unwrap();
    let html = String::from_utf8_lossy(&body).to_string();
    assert!(html.contains("浏览目录"), "正确密码应显示目录浏览入口: {}", html);
